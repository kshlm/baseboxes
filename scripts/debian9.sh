...
case "$PACKER_BUILDER_TYPE" in
		...
		apt-get --purge -y autoremove binutils cpp gcc kpartx $headers make
	...
	virtualbox-iso)
		...
		# Remove development tools.
		apt-get --purge -y autoremove binutils bzip2 cpp gcc $headers make
		...
		apt-get --no-install-recommends -y install fuse gcc $headers make net-tools perl
		...
		# Remove development tools.
		apt-get --purge -y autoremove binutils cpp gcc $headers make
